...
function integration::bazel_with_emulators() {
  ...
  production_integration_tests=(
    ...
    # Spanner gRPC/HTTP transcoding test
    "google/cloud/spanner/admin/integration_tests:instance_admin_rest_integration_test"
    "google/cloud/spanner/admin/integration_tests:database_admin_rest_integration_test"
  )
  ...
  bazel "${verb}" --test_output=all --cache_test_results=no "${args[@]}" --cache_test_results=no --test_tag_filters="${tag_filters}" \
    "${production_integration_tests[@]}"
  ...
  "google/cloud/spanner/ci/${EMULATOR_SCRIPT}" \
    bazel "${verb}" "${args[@]}" -c dbg --test_output=errors --cache_test_results=no
