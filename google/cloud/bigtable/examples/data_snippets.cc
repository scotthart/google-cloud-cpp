...
void PrepareAndExecuteQuery(google::cloud::bigtable::Client client,
  ...
  [](cbt::Client client, std::string const& project_id,
    ...
    std::cout << "project_id=" << project_id << "; instance_id=" << instance_id << "; table_id=" << table_id << std::endl;
...
//    cbt::SqlStatement statement(
//        "SELECT _key, CAST(fam['column0'] AS STRING) as value0 FROM `"
//            + table_id + "` WHERE _key='test-key-for-apply'", {});

    cbt::SqlStatement statement(
        "SELECT _key, CAST(fam['column0'] AS STRING) as value0 FROM `"
            + table_id + "` WHERE _key=@bob",
        {{"bob", cbt::Parameter(cbt::Bytes())}});

    auto prepared_query = client.PrepareQuery(instance, statement);
    if (!prepared_query) throw std::move(prepared_query).status();

    auto b = cbt::Bytes("test-key-for-apply");
    auto v = cbt::Value(b);
    std::cout << __func__ << "v=" << v.value().bytes_value() << std::endl;

    auto bound_query =
        prepared_query->BindParameters({{"bob", v}});
//                                         cbt::Value(cbt::Bytes("test-key-for-apply"))}});
//    auto bound_query =
//        prepared_query->BindParameters({});
    ...
    for (auto& row : cbt::StreamOf<RowType>(results)) {
      ...
      std::cout << std::get<0>(*row) << "; " << (v ? *v : "") << std::endl;
//      std::cout << std::get<0>(*row) << "; " << std::get<1>(*row) << "\n";
    }
    std::cout << "***** done printing rows *****" << std::endl;
  }
  // [prepare-and-execute-query]
...
void RunDataExamples(
  ...
  using ::google::cloud::Options;
  auto conn = cbt::MakeDataConnection();
  cbt::Table table(conn, cbt::TableResource(project_id, instance_id, table_id),
                   Options{}.set<cbt::IdempotentMutationPolicyOption>(
                       cbt::AlwaysRetryMutationPolicy().clone()));
