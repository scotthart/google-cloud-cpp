...
DirectPathProbeResult DirectPathProber::Probe(
  ...
  if (!auth) {
    result.status = internal::InvalidArgumentError(
        "Auth strategy cannot be null", GCP_ERROR_INFO());
    return result;
  }
  ...
  probe_options.set<EndpointOption>("google-c2p:///bigtable.googleapis.com");
  probe_options.set<AuthorityOption>("bigtable.googleapis.com");
  ...
  result.success = response.ok() && (is_alts || (is_auth && is_dp_ip));
