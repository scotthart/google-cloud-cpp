...
class DynamicChannelPool
 ...
 public:
  ...
  static std::shared_ptr<DynamicChannelPool> Create(
    ...
    std::cout << __PRETTY_FUNCTION__ << ": return pool size=" << pool->size()
              << std::endl;
    return pool;
  }
  ...
  std::shared_ptr<ChannelUsage<T>> GetChannelRandomTwoLeastUsed() {
    ...
    // typename std::vector<
    //     typename
    //     std::vector<std::shared_ptr<ChannelUsage<T>>>::iterator>::iterator
    ...
    if (d.channel_1_rpcs.ok() && d.channel_2_rpcs.ok()) {
      std::cout << __PRETTY_FUNCTION__ << ": 2 ok channels("
                << *d.channel_1_rpcs << ", " << *d.channel_2_rpcs
                << "), returning smaller" << std::endl;
      return *d.channel_1_rpcs < *d.channel_2_rpcs ? *d.channel_1_iter
                                                   : *d.channel_2_iter;
    }
    if (d.iterators.size() == 1 && d.channel_1_rpcs.ok()) {
      std::cout << __PRETTY_FUNCTION__
                << ": pool size is 1, returning OK channel" << std::endl;
      return *d.channel_1_iter;
    }
    if (d.iterators.empty()) {
      std::cout << __PRETTY_FUNCTION__
                << ": pool is empty, create and return channel" << std::endl;
 ...
 private:
  ...
  void RemoveChannels() {
    ...
    std::cout << __PRETTY_FUNCTION__
              << ": draining_channels_.size()=" << draining_channels_.size()
              << std::endl;
    while (!draining_channels_.empty()) {
      auto outstanding_rpcs =
          draining_channels_.back()->instant_outstanding_rpcs();
      std::cout << __PRETTY_FUNCTION__
                << ": outstanding_rpcs=" << *outstanding_rpcs << std::endl;
      if (outstanding_rpcs.ok() && *outstanding_rpcs > 0) {
        ScheduleRemoveChannels(lk);
        return;
      }
      std::cout << __PRETTY_FUNCTION__ << ": draining_channels_.pop_back()"
                << std::endl;
