...
DiagnosticFailureReason DirectPathDiagnostics::RunDiagnostics(
  ...
  if (!CanConnectTcp("169.254.169.254", 80, std::chrono::milliseconds(500))) {
...
void DirectPathDiagnostics::RunAsync(
    CompletionQueue cq, std::string const& /*client_uid*/,
  ...
  cq.RunAsync([options, direct_access_compatibility =
