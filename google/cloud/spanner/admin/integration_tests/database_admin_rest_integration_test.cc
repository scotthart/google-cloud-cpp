...
class DatabaseAdminClientRestTest
    : public ::google::cloud::testing_util::IntegrationTest {
 protected:
  static std::shared_ptr<google::cloud::spanner_admin::DatabaseAdminConnection>
  MakeDatabaseAdminConnectionRestEmulator(Options options) {
    Options orig = options;
    options =
        spanner_admin_internal::DatabaseAdminDefaultOptions(std::move(options));
    options.set<EndpointOption>(orig.get<EndpointOption>());
  ...
  void SetUp() override {
    std::cout << " emulator="
              << internal::GetEnv("SPANNER_EMULATOR_HOST").value() << std::endl;
    std::cout << " rest_emulator="
              << internal::GetEnv("SPANNER_EMULATOR_REST_HOST").value()
              << std::endl;
    using ::google::cloud::internal::GetEnv;
    emulator_ = GetEnv("SPANNER_EMULATOR_REST_HOST").has_value();
...
TEST_F(DatabaseAdminClientRestTest, DatabaseBasicCRUD) {
  std::cout << " rest_emulator=" << internal::GetEnv("SPANNER_EMULATOR_REST_HOST").value() << std::endl;
//  if (internal::GetEnv("SPANNER_EMULATOR_REST_HOST").has_value()) GTEST_SKIP();
...
TEST_F(DatabaseAdminClientRestTest, VersionRetentionPeriodCreate) {
  if (internal::GetEnv("SPANNER_EMULATOR_REST_HOST").has_value()) GTEST_SKIP();
...
TEST_F(DatabaseAdminClientRestTest, VersionRetentionPeriodCreateFailure) {
  if (internal::GetEnv("SPANNER_EMULATOR_REST_HOST").has_value()) GTEST_SKIP();
...
TEST_F(DatabaseAdminClientRestTest, VersionRetentionPeriodUpdate) {
  if (internal::GetEnv("SPANNER_EMULATOR_REST_HOST").has_value()) GTEST_SKIP();
  // Create the database.
  auto database =
      client_
...
TEST_F(DatabaseAdminClientRestTest, VersionRetentionPeriodUpdateFailure) {
  if (internal::GetEnv("SPANNER_EMULATOR_REST_HOST").has_value()) GTEST_SKIP();
  // Create the database.
  auto database =
      client_
...
TEST_F(DatabaseAdminClientRestTest, CreateWithEncryptionKey) {
...
/// supplied.
TEST_F(DatabaseAdminClientRestTest, CreateWithNonexistentEncryptionKey) {
  if (emulator_) GTEST_SKIP() << "emulator does not support CMEK";
  ...
  auto database = client_.CreateDatabase(creq).get();
  EXPECT_THAT(database, ::testing::AnyOf(StatusIs(StatusCode::kFailedPrecondition,
...
TEST_F(DatabaseAdminClientRestTest, DatabasePostgreSQLBasics) {
  if (internal::GetEnv("SPANNER_EMULATOR_REST_HOST").has_value()) GTEST_SKIP();
  google::spanner::admin::database::v1::CreateDatabaseRequest creq;
  creq.set_parent(database_.instance().FullName());
  creq.set_create_statement(
