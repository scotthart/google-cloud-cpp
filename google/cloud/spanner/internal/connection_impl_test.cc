...
TEST(ConnectionImplTest, MultiplexedPrecommitUpdated) {
  ...
  google::spanner::v1::MultiplexedSessionPrecommitToken token1;
  token1.set_precommit_token("test-precommit-token-1");
  token1.set_seq_num(1);
  ...
  //            EXPECT_FALSE(request.options().exclude_txn_from_change_streams());
  //            EXPECT_FALSE(request.has_mutation_key());
  //            return MakeTestTransaction(token1);
  //          });
  ...
  EXPECT_CALL(*mock, ExecuteSql)
      .WillOnce(Return(sql_response))
      .WillOnce(Return(sql_response2));
