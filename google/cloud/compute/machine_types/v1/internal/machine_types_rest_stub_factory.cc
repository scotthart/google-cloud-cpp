...
std::shared_ptr<MachineTypesRestStub> CreateDefaultMachineTypesRestStub(
  ...
  Options stub_creation_opts = rest_internal::DetermineStubCreationOptions(
      internal::GetEnv("GOOGLE_CLOUD_CPP_MACHINE_TYPES_ENDPOINT"),
      "compute.googleapis.com", options);

  std::shared_ptr<MachineTypesRestStub> stub =
      std::make_shared<DefaultMachineTypesRestStub>(
          std::move(stub_creation_opts));
  stub = std::make_shared<MachineTypesRestMetadata>(std::move(stub));
  if (internal::Contains(options.get<TracingComponentsOption>(), "rpc")) {
    GCP_LOG(INFO) << "Enabled logging for REST rpc calls";
    stub = std::make_shared<MachineTypesRestLogging>(
