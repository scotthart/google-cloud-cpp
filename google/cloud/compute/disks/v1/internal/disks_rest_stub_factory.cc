...
std::shared_ptr<DisksRestStub> CreateDefaultDisksRestStub(Options& options) {
  Options stub_creation_opts = rest_internal::DetermineStubCreationOptions(
      internal::GetEnv("GOOGLE_CLOUD_CPP_DISKS_ENDPOINT"),
      "compute.googleapis.com", options);

  std::shared_ptr<DisksRestStub> stub =
      std::make_shared<DefaultDisksRestStub>(std::move(stub_creation_opts));
  stub = std::make_shared<DisksRestMetadata>(std::move(stub));
  if (internal::Contains(options.get<TracingComponentsOption>(), "rpc")) {
    GCP_LOG(INFO) << "Enabled logging for REST rpc calls";
    stub = std::make_shared<DisksRestLogging>(
