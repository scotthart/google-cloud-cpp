...
#include "google/cloud/compute/zones/v1/internal/zones_rest_logging_decorator.h"
...
std::shared_ptr<ZonesRestStub> CreateDefaultZonesRestStub(Options& options) {
  Options stub_creation_opts = rest_internal::DetermineStubCreationOptions(
      internal::GetEnv("GOOGLE_CLOUD_CPP_ZONES_ENDPOINT"),
      "compute.googleapis.com", options);

  std::shared_ptr<ZonesRestStub> stub =
      std::make_shared<DefaultZonesRestStub>(std::move(stub_creation_opts));
  stub = std::make_shared<ZonesRestMetadata>(std::move(stub));
  if (internal::Contains(options.get<TracingComponentsOption>(), "rpc")) {
    GCP_LOG(INFO) << "Enabled logging for REST rpc calls";
    stub = std::make_shared<ZonesRestLogging>(
