...
std::shared_ptr<SubnetworksRestStub> CreateDefaultSubnetworksRestStub(
  ...
  Options stub_creation_opts = rest_internal::DetermineStubCreationOptions(
      internal::GetEnv("GOOGLE_CLOUD_CPP_SUBNETWORKS_ENDPOINT"),
      "compute.googleapis.com", options);

  std::shared_ptr<SubnetworksRestStub> stub =
      std::make_shared<DefaultSubnetworksRestStub>(
          std::move(stub_creation_opts));
  stub = std::make_shared<SubnetworksRestMetadata>(std::move(stub));
  if (internal::Contains(options.get<TracingComponentsOption>(), "rpc")) {
    GCP_LOG(INFO) << "Enabled logging for REST rpc calls";
    stub = std::make_shared<SubnetworksRestLogging>(
