...
TEST_F(ComputeIntegrationTest, PaginatedInstancesList) {
  ...
  for (auto instance : client.ListInstances(request)) {
    ...
    EXPECT_THAT(instance->name(), Not(IsEmpty()));
  }
}

TEST_F(ComputeIntegrationTest, GetInstances) {
  namespace instances = ::google::cloud::compute_instances_v1;
  auto client = instances::InstancesClient(
      google::cloud::ExperimentalTag{},
      instances::MakeInstancesConnectionRest(google::cloud::ExperimentalTag{}));
  ...
  auto result =
      client.GetInstances(project_id_, zone_, "test-e2-micro-instance");
