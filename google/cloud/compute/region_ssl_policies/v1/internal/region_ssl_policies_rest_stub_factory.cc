...
CreateDefaultRegionSslPoliciesRestStub(Options& options) {
  Options stub_creation_opts = rest_internal::DetermineStubCreationOptions(
      internal::GetEnv("GOOGLE_CLOUD_CPP_REGION_SSL_POLICIES_ENDPOINT"),
      "compute.googleapis.com", options);

  std::shared_ptr<RegionSslPoliciesRestStub> stub =
      std::make_shared<DefaultRegionSslPoliciesRestStub>(
          std::move(stub_creation_opts));
  stub = std::make_shared<RegionSslPoliciesRestMetadata>(std::move(stub));
  if (internal::Contains(options.get<TracingComponentsOption>(), "rpc")) {
    GCP_LOG(INFO) << "Enabled logging for REST rpc calls";
    stub = std::make_shared<RegionSslPoliciesRestLogging>(
