...
std::shared_ptr<RegionHealthChecksRestStub>
CreateDefaultRegionHealthChecksRestStub(Options& options) {
  Options stub_creation_opts = rest_internal::DetermineStubCreationOptions(
      internal::GetEnv("GOOGLE_CLOUD_CPP_REGION_HEALTH_CHECKS_ENDPOINT"),
      "compute.googleapis.com", options);

  std::shared_ptr<RegionHealthChecksRestStub> stub =
      std::make_shared<DefaultRegionHealthChecksRestStub>(
          std::move(stub_creation_opts));
  stub = std::make_shared<RegionHealthChecksRestMetadata>(std::move(stub));
  if (internal::Contains(options.get<TracingComponentsOption>(), "rpc")) {
    GCP_LOG(INFO) << "Enabled logging for REST rpc calls";
    stub = std::make_shared<RegionHealthChecksRestLogging>(
