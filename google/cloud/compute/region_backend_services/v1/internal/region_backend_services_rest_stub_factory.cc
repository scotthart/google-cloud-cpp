...
std::shared_ptr<RegionBackendServicesRestStub>
CreateDefaultRegionBackendServicesRestStub(Options& options) {
  Options stub_creation_opts = rest_internal::DetermineStubCreationOptions(
      internal::GetEnv("GOOGLE_CLOUD_CPP_REGION_BACKEND_SERVICES_ENDPOINT"),
      "compute.googleapis.com", options);

  std::shared_ptr<RegionBackendServicesRestStub> stub =
      std::make_shared<DefaultRegionBackendServicesRestStub>(
          std::move(stub_creation_opts));
  stub = std::make_shared<RegionBackendServicesRestMetadata>(std::move(stub));
  if (internal::Contains(options.get<TracingComponentsOption>(), "rpc")) {
    GCP_LOG(INFO) << "Enabled logging for REST rpc calls";
    stub = std::make_shared<RegionBackendServicesRestLogging>(
