...
std::shared_ptr<TargetTcpProxiesRestStub> CreateDefaultTargetTcpProxiesRestStub(
  ...
  Options stub_creation_opts = rest_internal::DetermineStubCreationOptions(
      internal::GetEnv("GOOGLE_CLOUD_CPP_TARGET_TCP_PROXIES_ENDPOINT"),
      "compute.googleapis.com", options);

  std::shared_ptr<TargetTcpProxiesRestStub> stub =
      std::make_shared<DefaultTargetTcpProxiesRestStub>(
          std::move(stub_creation_opts));
  stub = std::make_shared<TargetTcpProxiesRestMetadata>(std::move(stub));
  if (internal::Contains(options.get<TracingComponentsOption>(), "rpc")) {
    GCP_LOG(INFO) << "Enabled logging for REST rpc calls";
    stub = std::make_shared<TargetTcpProxiesRestLogging>(
