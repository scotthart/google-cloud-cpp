...
CreateDefaultTargetVpnGatewaysRestStub(Options& options) {
  Options stub_creation_opts = rest_internal::DetermineStubCreationOptions(
      internal::GetEnv("GOOGLE_CLOUD_CPP_TARGET_VPN_GATEWAYS_ENDPOINT"),
      "compute.googleapis.com", options);

  std::shared_ptr<TargetVpnGatewaysRestStub> stub =
      std::make_shared<DefaultTargetVpnGatewaysRestStub>(
          std::move(stub_creation_opts));
  stub = std::make_shared<TargetVpnGatewaysRestMetadata>(std::move(stub));
  if (internal::Contains(options.get<TracingComponentsOption>(), "rpc")) {
    GCP_LOG(INFO) << "Enabled logging for REST rpc calls";
    stub = std::make_shared<TargetVpnGatewaysRestLogging>(
