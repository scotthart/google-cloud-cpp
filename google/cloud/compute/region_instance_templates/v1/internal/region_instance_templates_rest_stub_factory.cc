...
CreateDefaultRegionInstanceTemplatesRestStub(Options& options) {
  Options stub_creation_opts = rest_internal::DetermineStubCreationOptions(
      internal::GetEnv("GOOGLE_CLOUD_CPP_REGION_INSTANCE_TEMPLATES_ENDPOINT"),
      "compute.googleapis.com", options);

  std::shared_ptr<RegionInstanceTemplatesRestStub> stub =
      std::make_shared<DefaultRegionInstanceTemplatesRestStub>(
          std::move(stub_creation_opts));
  stub = std::make_shared<RegionInstanceTemplatesRestMetadata>(std::move(stub));
  if (internal::Contains(options.get<TracingComponentsOption>(), "rpc")) {
    GCP_LOG(INFO) << "Enabled logging for REST rpc calls";
    stub = std::make_shared<RegionInstanceTemplatesRestLogging>(
