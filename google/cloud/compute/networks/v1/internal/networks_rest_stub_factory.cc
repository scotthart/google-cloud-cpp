...
std::shared_ptr<NetworksRestStub> CreateDefaultNetworksRestStub(
    Options& options) {
  Options stub_creation_opts = rest_internal::DetermineStubCreationOptions(
      internal::GetEnv("GOOGLE_CLOUD_CPP_NETWORKS_ENDPOINT"),
      "compute.googleapis.com", options);

  std::shared_ptr<NetworksRestStub> stub =
      std::make_shared<DefaultNetworksRestStub>(std::move(stub_creation_opts));
  stub = std::make_shared<NetworksRestMetadata>(std::move(stub));
  if (internal::Contains(options.get<TracingComponentsOption>(), "rpc")) {
    GCP_LOG(INFO) << "Enabled logging for REST rpc calls";
    stub = std::make_shared<NetworksRestLogging>(
