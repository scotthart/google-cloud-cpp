...
CreateDefaultInstanceTemplatesRestStub(Options& options) {
  Options stub_creation_opts = rest_internal::DetermineStubCreationOptions(
      internal::GetEnv("GOOGLE_CLOUD_CPP_INSTANCE_TEMPLATES_ENDPOINT"),
      "compute.googleapis.com", options);

  std::shared_ptr<InstanceTemplatesRestStub> stub =
      std::make_shared<DefaultInstanceTemplatesRestStub>(
          std::move(stub_creation_opts));
  stub = std::make_shared<InstanceTemplatesRestMetadata>(std::move(stub));
  if (internal::Contains(options.get<TracingComponentsOption>(), "rpc")) {
    GCP_LOG(INFO) << "Enabled logging for REST rpc calls";
    stub = std::make_shared<InstanceTemplatesRestLogging>(
