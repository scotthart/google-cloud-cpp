...
std::shared_ptr<TargetInstancesRestStub> CreateDefaultTargetInstancesRestStub(
  ...
  Options stub_creation_opts = rest_internal::DetermineStubCreationOptions(
      internal::GetEnv("GOOGLE_CLOUD_CPP_TARGET_INSTANCES_ENDPOINT"),
      "compute.googleapis.com", options);

  std::shared_ptr<TargetInstancesRestStub> stub =
      std::make_shared<DefaultTargetInstancesRestStub>(
          std::move(stub_creation_opts));
  stub = std::make_shared<TargetInstancesRestMetadata>(std::move(stub));
  if (internal::Contains(options.get<TracingComponentsOption>(), "rpc")) {
    GCP_LOG(INFO) << "Enabled logging for REST rpc calls";
    stub = std::make_shared<TargetInstancesRestLogging>(
