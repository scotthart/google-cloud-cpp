...
std::shared_ptr<RegionTargetHttpsProxiesRestStub>
CreateDefaultRegionTargetHttpsProxiesRestStub(Options& options) {
  Options stub_creation_opts = rest_internal::DetermineStubCreationOptions(
      internal::GetEnv("GOOGLE_CLOUD_CPP_REGION_TARGET_HTTPS_PROXIES_ENDPOINT"),
      "compute.googleapis.com", options);

  std::shared_ptr<RegionTargetHttpsProxiesRestStub> stub =
      std::make_shared<DefaultRegionTargetHttpsProxiesRestStub>(
          std::move(stub_creation_opts));
  stub =
      std::make_shared<RegionTargetHttpsProxiesRestMetadata>(std::move(stub));
  if (internal::Contains(options.get<TracingComponentsOption>(), "rpc")) {
    GCP_LOG(INFO) << "Enabled logging for REST rpc calls";
    stub = std::make_shared<RegionTargetHttpsProxiesRestLogging>(
