...
CreateDefaultGlobalPublicDelegatedPrefixesRestStub(Options& options) {
  ...
  std::shared_ptr<GlobalPublicDelegatedPrefixesRestStub> stub =
      std::make_shared<DefaultGlobalPublicDelegatedPrefixesRestStub>(
          std::move(stub_creation_opts));
  stub = std::make_shared<GlobalPublicDelegatedPrefixesRestMetadata>(
      std::move(stub));
  if (internal::Contains(options.get<TracingComponentsOption>(), "rpc")) {
    GCP_LOG(INFO) << "Enabled logging for REST rpc calls";
    stub = std::make_shared<GlobalPublicDelegatedPrefixesRestLogging>(
