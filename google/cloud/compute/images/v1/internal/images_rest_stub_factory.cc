...
std::shared_ptr<ImagesRestStub> CreateDefaultImagesRestStub(Options& options) {
  Options stub_creation_opts = rest_internal::DetermineStubCreationOptions(
      internal::GetEnv("GOOGLE_CLOUD_CPP_IMAGES_ENDPOINT"),
      "compute.googleapis.com", options);

  std::shared_ptr<ImagesRestStub> stub =
      std::make_shared<DefaultImagesRestStub>(std::move(stub_creation_opts));
  stub = std::make_shared<ImagesRestMetadata>(std::move(stub));
  if (internal::Contains(options.get<TracingComponentsOption>(), "rpc")) {
    GCP_LOG(INFO) << "Enabled logging for REST rpc calls";
    stub = std::make_shared<ImagesRestLogging>(
