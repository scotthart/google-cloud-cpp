...
CreateDefaultRegionNotificationEndpointsRestStub(Options& options) {
  ...
  std::shared_ptr<RegionNotificationEndpointsRestStub> stub =
      std::make_shared<DefaultRegionNotificationEndpointsRestStub>(
          std::move(stub_creation_opts));
  stub = std::make_shared<RegionNotificationEndpointsRestMetadata>(
      std::move(stub));
  if (internal::Contains(options.get<TracingComponentsOption>(), "rpc")) {
    GCP_LOG(INFO) << "Enabled logging for REST rpc calls";
    stub = std::make_shared<RegionNotificationEndpointsRestLogging>(
