...
std::shared_ptr<BackendBucketsRestStub> CreateDefaultBackendBucketsRestStub(
    Options& options) {
  Options stub_creation_opts = rest_internal::DetermineStubCreationOptions(
      internal::GetEnv("GOOGLE_CLOUD_CPP_BACKEND_BUCKETS_ENDPOINT"),
      "compute.googleapis.com", options);

  std::shared_ptr<BackendBucketsRestStub> stub =
      std::make_shared<DefaultBackendBucketsRestStub>(
          std::move(stub_creation_opts));
  stub = std::make_shared<BackendBucketsRestMetadata>(std::move(stub));
  if (internal::Contains(options.get<TracingComponentsOption>(), "rpc")) {
    GCP_LOG(INFO) << "Enabled logging for REST rpc calls";
    stub = std::make_shared<BackendBucketsRestLogging>(
