...
std::shared_ptr<FirewallsRestStub> CreateDefaultFirewallsRestStub(
  ...
  Options stub_creation_opts = rest_internal::DetermineStubCreationOptions(
      internal::GetEnv("GOOGLE_CLOUD_CPP_FIREWALLS_ENDPOINT"),
      "compute.googleapis.com", options);

  std::shared_ptr<FirewallsRestStub> stub =
      std::make_shared<DefaultFirewallsRestStub>(std::move(stub_creation_opts));
  stub = std::make_shared<FirewallsRestMetadata>(std::move(stub));
  if (internal::Contains(options.get<TracingComponentsOption>(), "rpc")) {
    GCP_LOG(INFO) << "Enabled logging for REST rpc calls";
    stub = std::make_shared<FirewallsRestLogging>(
