...
CreateDefaultRegionTargetHttpProxiesRestStub(Options& options) {
  Options stub_creation_opts = rest_internal::DetermineStubCreationOptions(
      internal::GetEnv("GOOGLE_CLOUD_CPP_REGION_TARGET_HTTP_PROXIES_ENDPOINT"),
      "compute.googleapis.com", options);

  std::shared_ptr<RegionTargetHttpProxiesRestStub> stub =
      std::make_shared<DefaultRegionTargetHttpProxiesRestStub>(
          std::move(stub_creation_opts));
  stub = std::make_shared<RegionTargetHttpProxiesRestMetadata>(std::move(stub));
  if (internal::Contains(options.get<TracingComponentsOption>(), "rpc")) {
    GCP_LOG(INFO) << "Enabled logging for REST rpc calls";
    stub = std::make_shared<RegionTargetHttpProxiesRestLogging>(
