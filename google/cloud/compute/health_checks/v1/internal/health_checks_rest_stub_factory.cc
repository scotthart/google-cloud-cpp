...
std::shared_ptr<HealthChecksRestStub> CreateDefaultHealthChecksRestStub(
  ...
  Options stub_creation_opts = rest_internal::DetermineStubCreationOptions(
      internal::GetEnv("GOOGLE_CLOUD_CPP_HEALTH_CHECKS_ENDPOINT"),
      "compute.googleapis.com", options);

  std::shared_ptr<HealthChecksRestStub> stub =
      std::make_shared<DefaultHealthChecksRestStub>(
          std::move(stub_creation_opts));
  stub = std::make_shared<HealthChecksRestMetadata>(std::move(stub));
  if (internal::Contains(options.get<TracingComponentsOption>(), "rpc")) {
    GCP_LOG(INFO) << "Enabled logging for REST rpc calls";
    stub = std::make_shared<HealthChecksRestLogging>(
