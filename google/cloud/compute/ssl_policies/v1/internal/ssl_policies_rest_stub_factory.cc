...
std::shared_ptr<SslPoliciesRestStub> CreateDefaultSslPoliciesRestStub(
  ...
  Options stub_creation_opts = rest_internal::DetermineStubCreationOptions(
      internal::GetEnv("GOOGLE_CLOUD_CPP_SSL_POLICIES_ENDPOINT"),
      "compute.googleapis.com", options);

  std::shared_ptr<SslPoliciesRestStub> stub =
      std::make_shared<DefaultSslPoliciesRestStub>(
          std::move(stub_creation_opts));
  stub = std::make_shared<SslPoliciesRestMetadata>(std::move(stub));
  if (internal::Contains(options.get<TracingComponentsOption>(), "rpc")) {
    GCP_LOG(INFO) << "Enabled logging for REST rpc calls";
    stub = std::make_shared<SslPoliciesRestLogging>(
