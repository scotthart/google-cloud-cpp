...
std::shared_ptr<InterconnectsRestStub> CreateDefaultInterconnectsRestStub(
  ...
  Options stub_creation_opts = rest_internal::DetermineStubCreationOptions(
      internal::GetEnv("GOOGLE_CLOUD_CPP_INTERCONNECTS_ENDPOINT"),
      "compute.googleapis.com", options);

  std::shared_ptr<InterconnectsRestStub> stub =
      std::make_shared<DefaultInterconnectsRestStub>(
          std::move(stub_creation_opts));
  stub = std::make_shared<InterconnectsRestMetadata>(std::move(stub));
  if (internal::Contains(options.get<TracingComponentsOption>(), "rpc")) {
    GCP_LOG(INFO) << "Enabled logging for REST rpc calls";
    stub = std::make_shared<InterconnectsRestLogging>(
