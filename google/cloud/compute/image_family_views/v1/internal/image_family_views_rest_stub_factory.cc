...
#include "google/cloud/compute/image_family_views/v1/internal/image_family_views_rest_metadata_decorator.h"
...
std::shared_ptr<ImageFamilyViewsRestStub> CreateDefaultImageFamilyViewsRestStub(
    Options& options) {
  Options stub_creation_opts = rest_internal::DetermineStubCreationOptions(
      internal::GetEnv("GOOGLE_CLOUD_CPP_IMAGE_FAMILY_VIEWS_ENDPOINT"),
      "compute.googleapis.com", options);

  std::shared_ptr<ImageFamilyViewsRestStub> stub =
      std::make_shared<DefaultImageFamilyViewsRestStub>(
          std::move(stub_creation_opts));
  stub = std::make_shared<ImageFamilyViewsRestMetadata>(std::move(stub));
  if (internal::Contains(options.get<TracingComponentsOption>(), "rpc")) {
    GCP_LOG(INFO) << "Enabled logging for REST rpc calls";
    stub = std::make_shared<ImageFamilyViewsRestLogging>(
