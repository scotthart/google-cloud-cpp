...
CreateDefaultInterconnectAttachmentsRestStub(Options& options) {
  Options stub_creation_opts = rest_internal::DetermineStubCreationOptions(
      internal::GetEnv("GOOGLE_CLOUD_CPP_INTERCONNECT_ATTACHMENTS_ENDPOINT"),
      "compute.googleapis.com", options);

  std::shared_ptr<InterconnectAttachmentsRestStub> stub =
      std::make_shared<DefaultInterconnectAttachmentsRestStub>(
          std::move(stub_creation_opts));
  stub = std::make_shared<InterconnectAttachmentsRestMetadata>(std::move(stub));
  if (internal::Contains(options.get<TracingComponentsOption>(), "rpc")) {
    GCP_LOG(INFO) << "Enabled logging for REST rpc calls";
    stub = std::make_shared<InterconnectAttachmentsRestLogging>(
