...
#include "google/cloud/compute/region_instances/v1/internal/region_instances_rest_logging_decorator.h"
...
std::shared_ptr<RegionInstancesRestStub> CreateDefaultRegionInstancesRestStub(
  ...
  Options stub_creation_opts = rest_internal::DetermineStubCreationOptions(
      internal::GetEnv("GOOGLE_CLOUD_CPP_REGION_INSTANCES_ENDPOINT"),
      "compute.googleapis.com", options);

  std::shared_ptr<RegionInstancesRestStub> stub =
      std::make_shared<DefaultRegionInstancesRestStub>(
          std::move(stub_creation_opts));
  stub = std::make_shared<RegionInstancesRestMetadata>(std::move(stub));
  if (internal::Contains(options.get<TracingComponentsOption>(), "rpc")) {
    GCP_LOG(INFO) << "Enabled logging for REST rpc calls";
    stub = std::make_shared<RegionInstancesRestLogging>(
