...
CreateDefaultRegionAutoscalersRestStub(Options& options) {
  Options stub_creation_opts = rest_internal::DetermineStubCreationOptions(
      internal::GetEnv("GOOGLE_CLOUD_CPP_REGION_AUTOSCALERS_ENDPOINT"),
      "compute.googleapis.com", options);

  std::shared_ptr<RegionAutoscalersRestStub> stub =
      std::make_shared<DefaultRegionAutoscalersRestStub>(
          std::move(stub_creation_opts));
  stub = std::make_shared<RegionAutoscalersRestMetadata>(std::move(stub));
  if (internal::Contains(options.get<TracingComponentsOption>(), "rpc")) {
    GCP_LOG(INFO) << "Enabled logging for REST rpc calls";
    stub = std::make_shared<RegionAutoscalersRestLogging>(
