...
std::shared_ptr<DiskTypesRestStub> CreateDefaultDiskTypesRestStub(
  ...
  Options stub_creation_opts = rest_internal::DetermineStubCreationOptions(
      internal::GetEnv("GOOGLE_CLOUD_CPP_DISK_TYPES_ENDPOINT"),
      "compute.googleapis.com", options);

  std::shared_ptr<DiskTypesRestStub> stub =
      std::make_shared<DefaultDiskTypesRestStub>(std::move(stub_creation_opts));
  stub = std::make_shared<DiskTypesRestMetadata>(std::move(stub));
  if (internal::Contains(options.get<TracingComponentsOption>(), "rpc")) {
    GCP_LOG(INFO) << "Enabled logging for REST rpc calls";
    stub = std::make_shared<DiskTypesRestLogging>(
