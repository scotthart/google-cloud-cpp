...
std::shared_ptr<VpnGatewaysRestStub> CreateDefaultVpnGatewaysRestStub(
    Options& options) {
  Options stub_creation_opts = rest_internal::DetermineStubCreationOptions(
      internal::GetEnv("GOOGLE_CLOUD_CPP_VPN_GATEWAYS_ENDPOINT"),
      "compute.googleapis.com", options);

  std::shared_ptr<VpnGatewaysRestStub> stub =
      std::make_shared<DefaultVpnGatewaysRestStub>(
          std::move(stub_creation_opts));
  stub = std::make_shared<VpnGatewaysRestMetadata>(std::move(stub));
  if (internal::Contains(options.get<TracingComponentsOption>(), "rpc")) {
    GCP_LOG(INFO) << "Enabled logging for REST rpc calls";
    stub = std::make_shared<VpnGatewaysRestLogging>(
