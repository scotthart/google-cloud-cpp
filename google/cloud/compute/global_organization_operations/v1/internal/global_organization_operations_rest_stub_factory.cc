...
CreateDefaultGlobalOrganizationOperationsRestStub(Options& options) {
  ...
  std::shared_ptr<GlobalOrganizationOperationsRestStub> stub =
      std::make_shared<DefaultGlobalOrganizationOperationsRestStub>(
          std::move(stub_creation_opts));
  stub = std::make_shared<GlobalOrganizationOperationsRestMetadata>(
      std::move(stub));
  if (internal::Contains(options.get<TracingComponentsOption>(), "rpc")) {
    GCP_LOG(INFO) << "Enabled logging for REST rpc calls";
    stub = std::make_shared<GlobalOrganizationOperationsRestLogging>(
