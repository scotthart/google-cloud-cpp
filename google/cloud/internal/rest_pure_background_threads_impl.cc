...
RestPureAutomaticallyCreatedBackgroundThreads::
    RestPureAutomaticallyCreatedBackgroundThreads(std::size_t thread_count)
...
void RestPureAutomaticallyCreatedBackgroundThreads::Shutdown() {
  cq_.Shutdown();
  for (auto& t : pool_) {
    ...
    try {
      ...
      GCP_LOG(FATAL)
          << "RestPureAutomaticallyCreatedBackgroundThreads::Shutdown: "
          << e.what();
    }
#endif
  }
