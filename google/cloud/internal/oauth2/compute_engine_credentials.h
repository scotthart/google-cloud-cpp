...
#include "google/cloud/internal/oauth2/credential_constants.h"
#include "google/cloud/internal/oauth2/credentials.h"
#include "google/cloud/internal/oauth2/refreshing_credentials_wrapper.h"
...
StatusOr<ServiceAccountMetadata> ParseMetadataServerResponse(
    internal::HttpResponse const& response);
...
ParseComputeEngineRefreshResponse(internal::HttpResponse const& response,
 ...
 */
template <typename HttpRequestBuilderType = internal::CurlRequestBuilder,
...
class ComputeEngineCredentials : public Credentials {
 ...
 private:
   ...
   */
  StatusOr<internal::HttpResponse> DoMetadataServerGetRequest(
      std::string const& path, bool recursive) const {
    // Allows mocking the metadata server hostname for testing.
    std::string metadata_server_hostname = internal::GceMetadataHostname();
