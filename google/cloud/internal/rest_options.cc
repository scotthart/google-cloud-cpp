...
using ChannelOptions = internal::ChannelOptions;
...
Options MakeOptions(RestOptions o) {
...
#if 0
RestOptions MakeBackwardsCompatibleRestOptions(Options opts) {
  return RestOptions(std::move(opts));
}
...
Options DefaultOptionsWithCredentials(Options opts) {
  ...
  if (opts.has<UnifiedCredentialsOption>()) {
    auto credentials = MapCredentials(opts.get<UnifiedCredentialsOption>());
    return internal::DefaultOptions(std::move(credentials), std::move(opts));
  }
  if (GetEmulator().has_value()) {
    return internal::DefaultOptions(
        MapCredentials(google::cloud::MakeInsecureCredentials()),
        std::move(opts));
  }
  return internal::DefaultOptions(
      MapCredentials(google::cloud::MakeGoogleDefaultCredentials()),
      std::move(opts));
}

// }  // namespace internal

StatusOr<RestOptions> RestOptions::CreateDefaultRestOptions() {
  return CreateDefaultRestOptions(ChannelOptions{});
}
...
StatusOr<RestOptions> RestOptions::CreateDefaultRestOptions(
  ...
  return RestOptions(*creds, channel_options);
}

RestOptions::RestOptions(std::shared_ptr<oauth2::Credentials> credentials,
                         ChannelOptions channel_options)
    : opts_(internal::DefaultOptions(std::move(credentials), {})),
      channel_options_(std::move(channel_options)) {}

RestOptions::RestOptions(Options o)
    : opts_(std::move(o)),
      user_agent_prefix_(
...
bool RestOptions::enable_http_tracing() const {
  return opts_.get<TracingComponentsOption>().count("http") != 0;
}

RestOptions& RestOptions::set_enable_http_tracing(bool enable) {
  if (enable) {
...
bool RestOptions::enable_raw_client_tracing() const {
  return opts_.get<TracingComponentsOption>().count("raw-client") != 0;
}

RestOptions& RestOptions::set_enable_raw_client_tracing(bool enable) {
  if (enable) {
...
RestOptions& RestOptions::SetDownloadBufferSize(std::size_t size) {
  opts_.set<DownloadBufferSizeOption>(
      size == 0 ? GOOGLE_CLOUD_CPP_STORAGE_DEFAULT_DOWNLOAD_BUFFER_SIZE : size);
  return *this;
}

RestOptions& RestOptions::SetUploadBufferSize(std::size_t size) {
