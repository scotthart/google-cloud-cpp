...
std::shared_ptr<JobsRestStub>
CreateDefaultJobsRestStub(Options const& options) {
  auto opts = internal::PopulateRestOptions(options);
  std::shared_ptr<JobsRestStub> stub =
      std::make_shared<DefaultJobsRestStub>(std::move(opts));
  stub = std::make_shared<JobsRestMetadata>(std::move(stub));
  if (internal::Contains(
      options.get<TracingComponentsOption>(), "rpc")) {
    GCP_LOG(INFO) << "Enabled logging for REST rpc calls";
    stub = std::make_shared<JobsRestLogging>(
        std::move(stub),
