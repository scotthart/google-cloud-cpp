...
std::shared_ptr<TablesRestStub>
CreateDefaultTablesRestStub(Options const& options) {
  auto opts = internal::PopulateRestOptions(options);
  std::shared_ptr<TablesRestStub> stub =
      std::make_shared<DefaultTablesRestStub>(std::move(opts));
  stub = std::make_shared<TablesRestMetadata>(std::move(stub));
  if (internal::Contains(
      options.get<TracingComponentsOption>(), "rpc")) {
    GCP_LOG(INFO) << "Enabled logging for REST rpc calls";
    stub = std::make_shared<TablesRestLogging>(
        std::move(stub),
