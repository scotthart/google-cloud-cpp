...
#include "google/cloud/bigquery/models/v2/internal/models_rest_stub_factory.h"
...
std::shared_ptr<ModelsRestStub>
CreateDefaultModelsRestStub(Options const& options) {
  auto opts = internal::PopulateRestOptions(options);
  std::shared_ptr<ModelsRestStub> stub =
      std::make_shared<DefaultModelsRestStub>(std::move(opts));
  stub = std::make_shared<ModelsRestMetadata>(std::move(stub));
  if (internal::Contains(
      options.get<TracingComponentsOption>(), "rpc")) {
    GCP_LOG(INFO) << "Enabled logging for REST rpc calls";
    stub = std::make_shared<ModelsRestLogging>(
        std::move(stub),
