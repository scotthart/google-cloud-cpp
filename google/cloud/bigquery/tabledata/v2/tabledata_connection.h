...
/// The retry policy for `TabledataConnection`.
class TabledataRetryPolicy : public ::google::cloud::RetryPolicy {
 public:
  /// Creates a new instance of the policy, reset to the initial state.
  virtual std::unique_ptr<TabledataRetryPolicy> clone() const = 0;
};

/**
 * A retry policy for `TabledataConnection` based on counting errors.
 *
 ...
 */
class TabledataLimitedErrorCountRetryPolicy : public TabledataRetryPolicy {
 public:
  /**
  ...
  std::unique_ptr<TabledataRetryPolicy> clone() const override {
    return std::make_unique<TabledataLimitedErrorCountRetryPolicy>(
        maximum_failures());
  }
...
/**
 * A retry policy for `TabledataConnection` based on elapsed time.
 *
 * This policy stops retrying if:
 * - An RPC returns a non-transient error.
 ...
 * - [`kUnavailable`](@ref google::cloud::StatusCode)
 */
class TabledataLimitedTimeRetryPolicy : public TabledataRetryPolicy {
 public:
  /**
  ...
  std::unique_ptr<TabledataRetryPolicy> clone() const override {
    return std::make_unique<TabledataLimitedTimeRetryPolicy>(
        maximum_duration());
  }
...
/**
 * The `TabledataConnection` object for `TabledataClient`.
 *
 * This interface defines virtual methods for each of the user-facing overload
 * sets in `TabledataClient`. This allows users to inject custom behavior
 * (e.g., with a Google Mock object) when writing tests that use objects of type
 * `TabledataClient`.
 *
 * To create a concrete instance, see `MakeTabledataConnection()`.
 *
 * For mocking, see `bigquery_tabledata_v2_mocks::MockTabledataConnection`.
 */
class TabledataConnection {
 public:
  virtual ~TabledataConnection() = 0;
...
#endif  // GOOGLE_CLOUD_CPP_GOOGLE_CLOUD_BIGQUERY_TABLEDATA_V2_TABLEDATA_CONNECTION_H
