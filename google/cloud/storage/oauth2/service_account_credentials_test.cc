...
#include "third_party/cloud_cpp/google/cloud/internal/random.h"
...
#include "third_party/cloud_cpp/google/cloud/storage/testing/mock_http_request.h"
...
/// @test Verify that nlohmann::json::parse() failures are reported as is_discarded.
TEST_F(ServiceAccountCredentialsTest, JsonParsingFailure) {
  std::string config = R"""( not-a-valid-json-string )""";
  // The documentation for nlohmann::json::parse() is a bit ambiguous, so wrote a
  // little test to verify it works as I expected.
  nlohmann::json parsed = nlohmann::json::parse(config, nullptr, false);
