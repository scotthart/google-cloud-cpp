...
struct MtlsObjectBasicCRUDIntegrationTest
 ...
 public:
  static Client MakeMtlsClient(std::string const& ssl_cert_blob,
    ...
    SslCertificate mtls_rest_config{ssl_cert_blob, ssl_key_blob};

    auto env = GetEnv("GOOGLE_CLOUD_CPP_REST_TEST_KEY_FILE_JSON");
    ...
    options.set<SslCertificateOption>(mtls_rest_config);
  ...
  static Client MakeMtlsClient(std::string const& ssl_client_cert_filename,
    ...
    SslCertificate mtls_rest_config{ssl_client_cert_filename, ssl_key_filename,
...
TEST_F(MtlsObjectBasicCRUDIntegrationTest, BasicCRUD) {
  ...
  //  if (!(GetEnv("SSL_CLIENT_CERT_FILENAME").has_value() &&
  //        GetEnv("SSL_KEY_FILENAME").has_value() &&
  //        GetEnv("SSL_KEY_FILE_PASSWORD").has_value())) {
  //    GTEST_SKIP();
  //  }
  //
  //  auto client = MakeMtlsClient(*GetEnv("SSL_CLIENT_CERT_FILENAME"),
  //                               *GetEnv("SSL_KEY_FILENAME"),
  //                               *GetEnv("SSL_KEY_FILE_PASSWORD"));

  // We'll need to leverage secret manager to plumb these env vars into the
  // test. If they aren't present skip this test for now.
  if (!(GetEnv("SSL_CLIENT_CERT_BLOB").has_value() &&
        GetEnv("SSL_KEY_BLOB").has_value())) {
    GTEST_SKIP();
  }

  auto client =
      MakeMtlsClient(*GetEnv("SSL_CLIENT_CERT_BLOB"), *GetEnv("SSL_KEY_BLOB"));
