...
std::shared_ptr<SqlUsersServiceRestStub> CreateDefaultSqlUsersServiceRestStub(
  ...
  Options stub_creation_opts = rest_internal::DetermineStubCreationOptions(
      internal::GetEnv("GOOGLE_CLOUD_CPP_SQL_USERS_SERVICE_ENDPOINT"),
      "sqladmin.googleapis.com", options);

  std::shared_ptr<SqlUsersServiceRestStub> stub =
      std::make_shared<DefaultSqlUsersServiceRestStub>(
          std::move(stub_creation_opts));
  stub = std::make_shared<SqlUsersServiceRestMetadata>(std::move(stub));
  if (internal::Contains(options.get<TracingComponentsOption>(), "rpc")) {
    GCP_LOG(INFO) << "Enabled logging for REST rpc calls";
    stub = std::make_shared<SqlUsersServiceRestLogging>(
