...
CreateDefaultDeprecatedServiceStub(
  ...
  auto service_grpc_stub = google::test::deprecated::v1::DeprecatedService::NewStub(channel);
  ...
  std::shared_ptr<DeprecatedServiceStub> stub =
    std::make_shared<DefaultDeprecatedServiceStub>(std::move(service_grpc_stub), std::move(service_operations_stub), std::move(service_iampolicy_stub), std::move(service_locations_stub));

  if (auth->RequiresConfigureContext()) {
    stub = std::make_shared<DeprecatedServiceAuth>(
        std::move(auth), std::move(stub));
  }
  stub = std::make_shared<DeprecatedServiceMetadata>(
      std::move(stub), std::multimap<std::string, std::string>{});
  if (internal::Contains(
      options.get<LoggingComponentsOption>(), "rpc")) {
    GCP_LOG(INFO) << "Enabled logging for gRPC calls";
    stub = std::make_shared<DeprecatedServiceLogging>(
        std::move(stub),
        options.get<GrpcTracingOptionsOption>(),
        options.get<LoggingComponentsOption>());
  }
  if (internal::TracingEnabled(options)) {
    stub = MakeDeprecatedServiceTracingStub(std::move(stub));
  }
  return stub;
}
