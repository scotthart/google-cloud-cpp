...
CreateDefaultGoldenRestOnlyRestStub(Options& options) {
  Options stub_creation_opts = rest_internal::DetermineStubCreationOptions(
      internal::GetEnv("GOOGLE_CLOUD_CPP_GOLDEN_REST_ONLY_ENDPOINT"),
      "goldenrestonly.googleapis.com",
      options);

  std::shared_ptr<GoldenRestOnlyRestStub> stub =
      std::make_shared<DefaultGoldenRestOnlyRestStub>(std::move(stub_creation_opts));
