...
WrapperGenerator::WrapperGenerator(
    ...
    google::cloud::generator_internal::VarsDictionary vars)
    : file_(file),
      context_(context),
      header_(context, vars["header_file_path"]),
      cc_(context, vars["cc_file_path"]),
      vars_(std::move(vars)) {}
...
#define $header_include_guard$
  ...
  for (int i = 0; i < file_->message_type_count(); ++i) {
    ...
    HeaderPrint(text->first, text->second);
    HeaderPrint("\n");
  }
...
Status WrapperGenerator::CcOpenNamespaces(NamespaceType ns_type) {
  auto namespace_value = OpenNamespaces(cc_, ns_type, "product_path", vars_);
  if (!namespace_value.ok()) return std::move(namespace_value).status();
  namespace_ = *namespace_value;
  return {};
}
...
StatusOr<WrapperGenerator> MakeWrapperGenerator(
  ...
  std::cerr << vars["header_file_path"] << "\n";
  std::cerr << vars["cc_file_path"] << "\n";
  return WrapperGenerator{file, context, std::move(vars)};
}
