...
clone();
  ...
  //    auto const& options = internal::CurrentOptions();
  //    if
  //    (options.has<$product_namespace$::$service_name$PollingPolicyOption>())
  //    {
  //      return
  //      options.get<$product_namespace$::$service_name$PollingPolicyOption>()->clone();
  //    }
  //    return
  //    options_.get<$product_namespace$::$service_name$PollingPolicyOption>()->clone();
  //  }
  //)""");
  //  }
...
std::string ConnectionImplRestGenerator::MethodDefinition(
    google::protobuf::MethodDescriptor const& method) {
  //  if (IsBidirStreaming(method)) {
  //    // We do not generate definitions for bidir streaming RPCs. Their retry
  //    or
  //    // resume loops are so custom (if possible at all), and their usage so
  //    rare,
  //    // that it is easier to hand-craft these functions in a streaming.cc
  //    file. return R"""()""";
  //  }
  //
  //  if (IsStreamingRead(method)) {
  ...
  //  auto& stub = stub_;
  //  auto retry = std::shared_ptr<$product_namespace$::$retry_policy_name$
  //  const>(retry_policy()); auto backoff = std::shared_ptr<BackoffPolicy
  ...
  //  };
  //  auto resumable =
  //      internal::MakeResumableStreamingReadRpc<$response_type$,
  //      $request_type$>(
  //          retry->clone(), backoff->clone(), [](std::chrono::milliseconds)
  //          {}, factory, $service_name$$method_name$StreamingUpdater,
  //          request);
...
$connection_impl_rest_class_name$::$method_name$($request_type$ request) {
  ...
  //        // internal::AsyncLongRunningOperation, a helper template function
  //        in
  ...
  //  auto& stub = stub_;
  //  return
  //  google::cloud::internal::AsyncLongRunningOperation<$longrunning_deduced_response_type$>(
  ...
  //    },)""",
  //        // One of the variations is how to extract the value from the
  //        operation
  //        // result, some operations use the metadata, some the data. We need
  //        to
  ...
  //        // Finally, the internal::AsyncLongRunningOperation helper may
  //        return
  //        // `future<StatusOr<google::protobuf::Empty>>`, in this case we add
  //        a
