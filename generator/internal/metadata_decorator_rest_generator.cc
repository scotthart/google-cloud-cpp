...
Status MetadataDecoratorRestGenerator::GenerateHeader() {
  ...
  HeaderPrint("\n");
  HeaderLocalIncludes(
      {vars("stub_rest_header_path"), "google/cloud/version.h"});
...
Status MetadataDecoratorRestGenerator::GenerateCc() {
  ...
  for (auto const& method : methods()) {
    ...
    //    }
    //    if (IsBidirStreaming(method)) {
    //      // Asynchronous streaming writes do not consume a request.
    //      Typically, the
    //      // first `Write()` call contains any relevant data to "start" the
    //      stream.
    //      // Thus, the decorator cannot add any routing instructions. The
    //      caller
  ...
  //    }
  //    if (IsStreamingWrite(method)) {
  //      // Asynchronous streaming writes do not consume a request. Typically,
  //      the
  //      // first `Write()` call contains any relevant data to "start" the
  //      stream.
