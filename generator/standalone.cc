...
// Standalone expects proto package as an argument. It defines the API
// for which the code must be generated for (i.e. to distinguish between
...
// Converts arguments from what is required by the generator standalone mode
...
// preferred way of writing the generator in C++:
...
bool ConvertCommandLineArgs(int argc, char const* const argv[],
                            ...
                            std::string* error_msg) {
  // Generator Standalone arguments
  ...
  for (int i = 0; i < argc; i++) {
    ...
    if (arg_name == desc_arg || arg_name == desc_set_in_arg) {
      ...
      args->emplace_back("--cpp_codegen_out=" + arg_val);
